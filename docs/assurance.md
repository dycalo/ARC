...
Repository evaluations support an explicit per-run output-token cap, defaulting to 16,384. The driver and host gateway enforce the selected ceiling, and native compaction is capped at the smaller of 8,192 and that ceiling. This allowance includes reasoning tokens and is separate from exact View bytes and input reservations. Lowering it can admit another request within an existing budget but can also truncate model output; it does not automatically recover a budget-stopped task.
