...
Native DSH pruning and other host rewrites of retained tool results produce replacement chains that ARC does not currently reconcile. These paths stop before further model dispatch; they do not gain authority through a valid DSH surface replacement marker.
...
The declarative adapter gives only its two newest progress candidates priority over other archived evidence. Older progress remains eligible later in the candidate order or through explicit requirements. This avoids allocating every optional slot to model prose, without granting it observation authority or changing dependency and expiry checks.
