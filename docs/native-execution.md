...
Each response permits 1–16 advertised native wrappers in one sequential plan, or a single `arc_act` in a separate response. Mixing managed and native calls, using `arc_step`, or calling unwrapped native tools is unavailable in this interface. Whole-response validation precedes the first effect; declarations activate only after the complete native plan settles. Wrappers execute the original registered native tool through DSH's public pipeline; the same journal, immutable observations and durable-receipt settlement apply. Changing between the two declarative interfaces can reconcile pending results without replaying native operations. Existing saved configurations retain their selection; the default remains the batch interface.
...
Optional [`nativeHistorySteps`](configuration.md#bounded-native-conversation) can retain a short suffix of native responses and tool messages containing actual admitted output. Complete assistant captures, original receipts and native output records must first enter the current View. DSH surface replacements show source-labelled native output in the corresponding tool message without changing its call identity or error status. Success receipts remain in the execution journal; original outer error feedback is preserved. Earlier faithful receipt-plus-output projections remain valid on recovery. Original append events alone establish execution and settlement; replacements are checked against those events and their settled plans. This preserves conversation roles under the same full-request cap and fresh per-call certificates; it adds no tool execution and cannot extend source validity. Default zero keeps the existing View-only conversation.
