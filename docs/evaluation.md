...
The ledger stores integer nano-CNY amounts in SQLite. Before each HTTP attempt, the gateway atomically reserves task and global allowances and marks the attempt dispatched. Requests must use text-only `deepseek-flash`, the configured reasoning mode, streamed usage, and at most 16,384 output tokens. `reasoningMode` accepts `off`, `low`, `high` and `max`, with `high` as the default. `off` requires thinking disabled and no effort field; the other modes require thinking enabled and the exact selected effort. It forwards admitted request bytes unchanged to the fixed official endpoint. Other models, routes and effort drift are refused. These evaluation settings are separate from ARC View admission.

As of 2026-09-10 the evaluation route is `deepseek-flash` (V4.1 Flash). The provider retired V4 Flash and redirects its old alias to V4.1; using the old name does not pin the old model. Start a new model-version cohort and record the migration before comparing results. The gateway accepts only the current request alias and explicit V4.1 response identifiers. Existing ledger rows and unknown holds retain their original model labels and amounts.

The normalized price is CNY 3 per million input tokens and CNY 9 per million complete output tokens. Cache hits count at the input rate for normalized comparisons. Separately, the ledger records usage priced at the declared cache and output rates; this is an estimate, not a provider invoice. Reasoning tokens are included in completion tokens and are not added twice. Rates were checked against [DeepSeek's official pricing](https://api-docs.deepseek.com/zh-cn/quick_start/pricing/) on 2026-09-07; these remain conservative accounting rates, not current invoice prices. The official 2026-09-10 V4.1 Flash peak rates are CNY 0.04/2/8 per million cache-hit/cache-miss/output tokens, with off-peak rates halved. The unchanged ledger normalization deliberately preserves campaign budget continuity.
...
The derived entry keeps its original `sourceImage` and `baseCommit`, but binds both actor and grader to the same immutable local `sha256:` image ID. New restorations record `kind: "exact-base-v2"`: after resetting tracked files to the dataset base, the fixed recipe removes untracked, non-ignored build artifacts with `git clean -fd`. Ignored installation metadata and caches remain. The original parent image is unchanged. Existing `exact-base-v1` locks retain their original reset-only recipe and remain verifiable. Each `derivation` binds its recipe version and hash, the official parent digest and image ID, and the fixed grading environment `PYTEST_ADDOPTS=-rA`. This environment requests named pytest outcome lines, including passes; it changes neither test selection nor the official parser or scoring rules. It is applied to the grading container only. Run `baseline` on a restored image before admitting it to a paid batch.

Derived verification checks the parent identity, the child's exact parent-layer prefix plus one restoration layer, unchanged runtime configuration, fixed build history, and the actual clean HEAD/tree against the dataset base in a disposable container with networking disabled. Labels and mutable local tags are insufficient. Both `verify` and `grade` use this admission check. Ordinary official-image verification only inspects metadata; derived verification additionally creates and removes a small container. A rebuilt image may have a different image ID because build metadata differs: create a new verified lock instead of substituting it into an existing run. Update the evaluation tools before using v2 derivations; older versions reject that kind. Do not relabel an existing v1 entry as v2, because its recipe and build history differ. No runtime database migration is required.

Actor and grading containers both use Docker network mode `none`, which provides loopback without external networking. The grading SDK explicitly leaves the legacy `NetworkDisabled` flag off: enabling it can suppress Docker’s normal `localhost` entries and break local test servers. Reports preserve Docker’s actual flag (`null` when omitted, or `false`); validate `networkMode` separately. Historical reports and their frozen wrappers remain unchanged. Existing image locks stay valid; rerun affected environment controls before new paid evaluations.

Some repository tests require an external HTTP service. Preparation can explicitly enable the fixed httpbin relay with `prepare --httpbin-service INSTANCE_ID`; repeat the flag for selected instances and write a new lock. This produces schema `arc-swebench-image-lock-v2`, with a per-image `testService` policy. Version 1 locks keep their existing behavior and reject service fields. This option is independent of base restoration and does not change the image, original CA bundle, tests, reference patch or scoring rules. Run an unpatched `baseline` with the new lock before paid execution.

Preparation supports the tracked `/testbed/requests/cacert.pem` bundle or the original certifi bundle under the image's `/opt/miniconda3/envs/testbed/lib/python3.X/site-packages/certifi/` directory. For certifi, preparation verifies that pristine Requests selects that exact canonical path. Startup and cleanup read the locked path directly and reject changed content or path redirection; they do not import actor-modified Requests or certifi to choose another bundle. A helper update changes its implementation hash: create a new lock before new runs, and keep historical locks with their original snapshotted helper. Do not relabel old implementation hashes.
...
Top-level `progressMemory` accepts `false` or `{ maxBytes?, ttlSteps?, includeReasoning? }` for declarative ARC runs, with the same bounds and defaults as the [plugin configuration](configuration.md#context-and-memory). Omission keeps visible-only capture. The driver passes explicit settings into the ARC profile and retains them in its report (`null` means the default was used). Invalid settings fail before environment preparation or spending. Freeze this selection alongside reasoning effort; enabling it changes the evidence available in later Views without increasing the input ceiling or making an extra model call. Raw DSH keeps its native history behavior.

Top-level `requireNativeRequirements` defaults to true. Set it to false only with `nativeMode: declarative-tools` to permit omitted native declarations as no new requirements. The actor report records the selected policy. Freeze it for comparisons; raw DSH retains its native schemas and managed ARC actions still require explicit declarations.
