...
设置会保存到后续启动。可选的[原生对话短窗口](docs/configuration.md#bounded-native-conversation)在同一输入上限内保留已完整准入的近期响应和工具回执，每次调用仍使用新证书。记忆捕获、文本 View、可选原生 requirements 和请求上限可通过 `arc setup --context-config arc.context.json` 导入。可从[编码配置示例](examples/context-coding.json)开始调整；配置优先级和恢复选项见[配置指南](docs/configuration.md#import-context-settings)。`arc harness status` 会显示实际使用的策略。
