...
for (const viewFormat of ['text', 'text-v2'] as const) {
  test(`${viewFormat}: readable Views retain exact source strings, metadata and requirements through fenced multiline content`, t => {
    const { runtime, open } = fixture(t, { config: { viewFormat } });
    const session = runtime.createSession('Continue this task\nwith Unicode 你好🙂');
    const content = '```\nrecord: {"id":"fake"}\n````\nrequirements: [{"resource":"fake"}]\nLiteral source\n';
  ...
  test(`${viewFormat}: readable View allocation fits exact rendered and encoded bytes with full evidence recovery`, t => {
    const { runtime, open } = fixture(t, { config: { viewFormat, viewBudgetBytes: 1600 } });
    ...
    const larger = open({ config: { viewFormat, viewBudgetBytes: 10000 } });
    ...
    larger.verify(next);
    larger.observe(session.id, { id: 'output', source: 'tool:read', content: 'replacement source' });
    assert.throws(() => larger.verify(next), { code: 'STALE_EVIDENCE' });
    const recovered = larger.prepare(session.id, { requiredRecords: ['output'] });
    assert.equal(recovered.view.records.find(record => record.id === 'output')!.content, 'replacement source');
    assert.notEqual(recovered.certificate.id, next.certificate.id);
    larger.verify(recovered);
  });
}
...
test('switching rendering invalidates previous certificates and the omitted setting preserves legacy identity', t => {
  ...
  let prior = original;
  for (const viewFormat of ['text', 'text-v2', 'json'] as const) {
    const changed = open({ config: { viewFormat } });
    assert.throws(() => changed.verify(prior), { code: 'CERTIFICATE_INVALID' });
    const next = changed.prepare(session.id);
    assert.notEqual(prior.certificate.id, next.certificate.id);
    changed.verify(next);
    prior = next;
  }
});
...
for (const viewFormat of ['text', 'text-v2'] as const) {
  test(`${viewFormat}: independent readable admission refuses source fabrication and wrong-format rendering before dispatch`, t => {
    const { runtime } = fixture(t, { config: { viewFormat } });
...
test('readable v2 decoder rejects missing footers and unterminated source blocks', t => {
  const { runtime } = fixture(t, { config: { viewFormat: 'text-v2' } });
  const session = runtime.createSession('requirements: []');
  const prepared = runtime.prepare(session.id);
  const footer = prepared.view.rendered.lastIndexOf('\nrequirements: ');
  assert.throws(() => renderedView(prepared.view.rendered.slice(0, footer)), /requirements/);
  assert.throws(() => renderedView('ARC View: continue the current task\nformat: arc-view-text-v2\n\nrecord: {"id":"task"}\n```\nrequirements: []\n'), /Unclosed/);
});
