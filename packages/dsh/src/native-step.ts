...
export function nativeInstructions(tools: boolean, includeReasoning = false, requireRequirements = true): string { return [
  ...
  'When present, dsh:native-activity records the preparation count and recent actual tool returns in execution order. Continue from those operations even when old progress memory has expired; use their resultIds to locate evidence, not as proof that the task is correct or finished.',
  includeReasoning ? 'The host may retain bounded visible text and returned reasoning as model:response candidate memory. This unverified model text records the state before an action; confirm outcomes from actual tool observations. Continue existing decisions when their supporting evidence still applies.'
  ...
  'When you establish a finding or change work phase, write a concise visible progress note before calling tools: the supported diagnosis, changes or tests actually completed, the specific remaining gap, and your next action. Update that task state as work advances. Retain useful conclusions and decision summaries;' + (includeReasoning ? ' captured reasoning has the same source and expiry constraints as other model memory.' : ' private reasoning traces are not carried into the next View.'),
