...
export function nativeInstructions(tools: boolean, includeReasoning = false, requireRequirements = true): string { return [
  ...
  'Treat model memory as unverified candidate text with its original sources and expiry.' + (includeReasoning ? ' Captured returned reasoning has the same restrictions.' : ' Private reasoning traces are not retained.') + ' When a useful conclusion or work phase changes, write a short visible progress note with verified findings and the next action. No manual checkpoint or goal is needed unless an admitted host checkpoint policy requires it.',
