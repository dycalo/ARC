...
try {
  ...
  const result = await runTask({
    ...settings,
    workspace,
    task: 'Set the managed database key release.canary to the JSON number 7 using the set action. Then finish with the exact summary ARC live provider smoke passed. Do not access files or other data.',
  });
  ...
  if (result.session.status !== 'completed' || value !== 7) throw new Error('Live smoke did not finish the managed task correctly.');
